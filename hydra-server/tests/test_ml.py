...
class TestMarkovGenerator:
    def test_train_and_generate(self) -> None:
...
class TestRuleClassifier:
    def test_heuristic_predict(self) -> None:
        clf = RuleClassifier()
        rules = clf.predict("password", top_k=5)
        assert len(rules) > 0
        assert len(rules) <= 5

    def test_heuristic_predict_uppercase(self) -> None:
        clf = RuleClassifier()
        rules = clf.predict("PassWord", top_k=3)
        assert "l" in rules

    def test_heuristic_predict_digits(self) -> None:
        clf = RuleClassifier()
...
class TestStrategySelector:
    ...
    def test_fast_hash_gets_many_phases(self) -> None:
        phases = self.sel.select_phases(HashType.MD5)
        assert len(phases) >= 4

    def test_slow_hash_gets_few_phases(self) -> None:
        phases = self.sel.select_phases(HashType.BCRYPT)
        assert len(phases) <= 2

    def test_preferred_engine_slow(self) -> None:
        engine = self.sel.get_preferred_engine(HashType.BCRYPT)
        assert engine == "john"

    def test_preferred_engine_fast(self) -> None:
        engine = self.sel.get_preferred_engine(HashType.MD5)
        assert engine == "hashcat"

    def test_chunk_size_scales_with_speed(self) -> None:
    ...
    def test_markov_only_for_slow_with_data(self) -> None:
